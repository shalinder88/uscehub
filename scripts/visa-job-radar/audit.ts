...
async function main(): Promise<void> {
  ...
  lines.push(``);
  lines.push(`12. **Mass General Brigham Workday connector + "physican" PHYS_TOKEN** — FIXED run 0217: workday-mgb connector added (massgeneralbrigham/wd1/MGBExternal). 6yr/73pos iron-core; direct DOL normKey match ("mass general brigham"), no alias needed. "physican" (missing 'i') added to PHYS_TOKENS — MGB consistently uses this typo in ATS job titles (e.g. "Physican Urology", "Physican-Pediatrics"); 8-char substring ≠ 9-char "physician", so these were being silently dropped. 21 clean SPONSOR_LEAD surfaced (Physician Scientist, Physician Internal Medicine, Physician Urology, Infectious Disease Physician, Physican-Pediatrics, and 16 more attending-level roles). No new false-positives found in MGB sample.`);
  lines.push(``);
  lines.push(`13. **MUSC Workday connector + employer normKey conflict fix** — FIXED run 0257: workday-musc connector added (musc/wd1/MUSC). 7yr/49pos iron-core (Medical University of South Carolina and Affiliates). Initial source.employer = "Medical University of South Carolina" normalized to "medical university of south carolina" — blocked by a pre-existing 1yr/0pos SPONSOR_DATA entry with the same normKey; alias guard (!m.has(atsKey)) prevented override. Fix: source.employer set to exact DOL legal name "Medical University of South Carolina and Affiliates" → normEmployer() = "medical university of south carolina and affiliates" = direct iron-core match. 20 clean SPONSOR_LEAD surfaced (all UNIV-prefix faculty: Neurosurgery, Neurohospitalist, Critical Care Anesthesiology, Surgical Pathology, Interventional Radiology, Breast Imaging Radiology, Neurology, Psychiatry, General Internal Medicine). 1 correctly SPONSORSHIP_DENIED (PRN Neurologist). CXS limit=100 appears to hit a server cap — use limit≤20 per page.`);
  lines.push(``);
  lines.push(`14. **Brown Health Workday connector + "physician liaison" NONPHYS_TOKEN + Workday "provider" facet fix** — FIXED run 0324: Three issues. (1) workday-brownhealth connector added (brownhealth/wd12/External_Careers). Brown Health = formerly Lifespan Health System, rebranded ~2023. EMPLOYER_ALIAS "brown health" → "lifespan physician" (7yr/48pos iron-core; no normKey collision — "brown health" not in persistence_index). (2) "physician liaison" added to NONPHYS_TOKENS — Brown Health uses "Sr. Physician Liaison" (non-MD outreach/recruitment role). (3) Workday physicianFacetIds() had "provider" in match terms — Brown Health exposes an "Advanced Practice Provider" job family with d.includes("provider") = true; facet probe selected it (54 NP/PA/APP jobs) and used it as the filter, returning 0 real physicians (keyword fallback never activated). Fix: removed "provider" from physicianFacetIds; audited all 14 active Workday connectors — none uses a "provider" job family for physician jobs. 27 clean SPONSOR_LEAD surfaced: physician, Physician, MG Physician, Physician PD, Staff Physician, Hospitalist, Pediatrician, Physician Gen Cardiology, Physician - Primary Care, Staff Physician Emergency Medicine.`);
  lines.push(``);
