        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-white dark:bg-slate-900 p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100 dark:text-slate-100">Report Your Match</h3>
              <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-slate-600 dark:text-slate-400 dark:text-slate-400">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            {submitted ? (
              <div className="py-8 text-center">
                ...
                <p className="mt-2 font-medium text-slate-900 dark:text-slate-100 dark:text-slate-100">Congratulations on your match!</p>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 dark:text-slate-400">Thank you for sharing.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-3">
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400 dark:text-slate-400">Name (optional)</label>
                  <input
                    type="text"
                    value={form.name}
                    onChange={(e) => setForm({ ...form, name: e.target.value })}
                    placeholder="Your name"
                    className="w-full rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400 dark:text-slate-400">Program Matched *</label>
                  <input
                    type="text"
                    value={form.program}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400 dark:text-slate-400">Match Year *</label>
                    <select
                      value={form.year}
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-600 dark:text-slate-400 dark:text-slate-400">USCEHub Program Used *</label>
                    <input
                      type="text"
