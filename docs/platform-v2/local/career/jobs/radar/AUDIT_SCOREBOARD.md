...
20. **New disabled connectors ready for wd5 recovery (2026-06-13)**: `workday-urmc` (rochester/wd5/UR_Staff, 35pos + Unity Hospital 24pos) and `workday-rochestergeneral` (rrhs/wd5/RRH, 110pos) added to registry as DISABLED. Re-enable when wd5 CDN recovers. Expected yield: 40-80 additional SPONSOR_LEAD combined. Also added disabled stubs `jibe-ufhealth` (pinned, 90pos) and `jibe-medstar` (pinned, 115pos combined) documenting exhausted Jibe explorations. Additionally confirmed: Banner Health `banner/wd1` and Baystate Medical `baystatehealth/wd1` tenants exist (422 from CXS) — site paths needed; add when wd1 recovers. Broad iron-core ATS sweep (25+ employers probed): Mayo/Henry Ford/IU Health/SUNY Upstate/Northwell/UAB all blocked (Infor/Phenom/iCIMS); OHSU/MedStar Jibe portals pinned; no new enabled connectors added.
